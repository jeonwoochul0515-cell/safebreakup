...
## Phase 11: 가격 정책 확정 + 결제 시스템

> 현재 상태: 가격 문구만 있고 결제 플로우 미구현

### 11-1. 현재 문제

| 문제 | 상세 |
|------|------|
| **결제 시스템 자체 없음** | 가격 문구만 산재, 실제 결제 불가 |
| **가격 불일치** | 홈: "월 4,900원" / AI사무장: "49,000원/99,000원" / research.md: "9,900원/29,900원 제안" |
| **삭제된 유료 화면** | "99,000원 증거 분석 보고서" 문구가 법률 챗봇에 남아있으나 해당 화면은 리다이렉트 |
| **구독 내용 미정의** | "월 4,900원"이 뭘 포함하는지 불명확 |
| **경호 서비스 가격 미노출** | 어드민에만 존재 (사용자 화면에 없음) |

### 11-2. 확정할 가격 구조 (우철님 결정 필요)

**확정: 2티어 + 건별 과금**

| 티어 | 월 가격 | 포함 내용 |
|------|:-------:|-----------|
| **무료** | 0원 | 증거 보관, SOS, 법률 정보 챗봇(일 3회), 자가진단, 은밀모드 |
| **스탠다드** | 9,900원 | AI 챗봇 무제한, 고소장/경고장 **AI 초안 (변호사 검토 없음)**, 증거 AI 분석, 보호 점수, 클라우드 백업 |

**스탠다드 책임 한계 고지 (필수 표시):**
> "스탠다드 플랜의 고소장/경고장은 AI가 자동 생성한 초안이며, 변호사의 검토를 거치지 않았습니다. 법적 효력을 보장하지 않으며, 정확한 법률 조언을 위해서는 별도 변호사 상담(건별 유료)을 이용해 주세요. 본 서류를 경찰/법원에 제출하기 전 반드시 전문가 검토를 권장합니다."

**변호사 검토가 필요한 서비스 = 건별 유료:**

| 서비스 | 가격 | 포함 내용 |
|--------|:----:|-----------|
| 변호사 상담 10분 | 70,000원 | 김창희 변호사 직접 상담 |
| 고소장 변호사 검토 | 199,000원 | AI 초안 + 변호사 수정/확정 |
| 내용증명 발송 (우편) | 99,000원 | 변호사 명의 작성 + 우체국 발송 |
| 경고장 발송 (이메일/SNS) | 49,000원 | 변호사 명의 작성 + 발송 |
| 경호 서비스 | 150,000원~ | 전문 경호원 현장 동행 |
...
|--------|:----:|
| 변호사 상담 10분 | 70,000원 |
| 추가 고소장 (변호사 검토) | 199,000원 |
| 내용증명 발송 (우편) | 99,000원 |
| 경고장 발송 (이메일/SNS) | 49,000원 |
| 경호 서비스 | 150,000원~ |
...
| 파일 | 수정 내용 |
|------|-----------|
| `app/(tabs)/index.tsx` | 프리미엄 배너 가격 업데이트 |
| `app/(tabs)/legal-info.tsx` | "99,000원 증거 분석 보고서" 문구 제거/수정 |
| `app/subscription.tsx` | 3티어 비교표 추가 |
| `app/landing.tsx` | 가격표 업데이트 |
| `lib/ai-secretary.ts` | 경고장 가격 문구 통일 |
| `constants/legal.ts` | 가격 상수 중앙 관리 신규 추가 |
| `types/database.ts` | subscription_tier → 'free' \| 'standard' (premium 삭제) |

### 11-4. 구현 순서

| 단계 | 작업 |
|:----:|------|
| **P11-1** | `constants/pricing.ts` 신규 — 가격 상수 중앙 관리 |
| **P11-2** | 모든 가격 문구를 상수에서 import하도록 변경 |
| **P11-3** | `app/subscription.tsx` 3티어 비교표 UI |
| **P11-4** | 결제 플로우 설계 (Expo IAP 또는 Stripe) |
| **P11-5** | 구독 상태 AsyncStorage/Supabase 연동 |
...
|:-----:|:------:|:----:|
| 1~6 | **100%** | ✅ 완료 |
| 7 (통합 리팩토링) | **70%** | ⚠️ 리다이렉트만, 파일 미삭제 |
| 8 (감사 수정) | **75%** | ⚠️ 암호화 방식 불일치, 동의 검증 미완 |
| 9 (보호 현황) | **60%** | ⚠️ 데이터 연동만, 멀티트랙 UI 미구현 |
| 10 (파일 업로드/AI) | **65%** | ⚠️ Mock 분석만, API 미연동 |

### 12-2. 잔여 미구현 항목 (우선순위 순)

#### 🔴 출시 차단 (Critical)

| # | 항목 | 현재 | 필요 |
|:-:|------|------|------|
| 1 | **암호화 방식 명시** | XOR 스트림 (계획은 AES-256) | plan에 현 방식 인정하거나 AES로 교체 |
| 2 | **삭제된 화면 가격 문구** | 법률 챗봇에 "99,000원 보고서" 남아있음 | 문구 제거 또는 /complaint로 연결 |
| 3 | **동의 검증 로직** | ConsentModal 있으나 일부 화면에서 미호출 | 전 화면 동의 체크 활성화 |

#### 🟠 높음

| # | 항목 | 현재 | 필요 |
|:-:|------|------|------|
| 4 | **홈 "증거 포렌식" 카드** | `/evidence-forensics` → 그냥 증거함으로 | 카드 제거 또는 설명 변경 |
| 5 | **constants/police-report.ts** | 파일 존재 | 사용처 확인 후 삭제 |
| 6 | **멀티 트랙 UI** | caseType만 추가, 트랙 표시 안 됨 | StatusTracker에 트랙별 단계 표시 |
| 7 | **보호 플랜 상태** | `useState('FREE')` 하드코딩 | 구독 상태 연동 |
| 8 | **2023도6411 판례 멘트** | 상수에만 있음 | 사무장 대화에 반영 |

#### 🟡 중간

| # | 항목 | 현재 | 필요 |
|:-:|------|------|------|
| 9 | Claude API 실제 연동 | Mock 키워드 분석 | API 호출 구현 |
| 10 | Whisper STT 연동 | 스텁만 존재 | 음성 분석 구현 |
| 11 | AI 분석 결과 UI | 필드 정의만 | 증거 카드에 표시 |
| 12 | 가격 상수 중앙화 | 여러 파일에 산재 | `constants/pricing.ts` 생성 |
| 13 | `constants/letter-templates.ts` 정리 | 존재 | 중복 확인 후 정리 |

### 12-3. plan.md vs research.md 불일치

| 항목 | research.md | plan.md | 코드 | 조치 |
|------|:-----------:|:-------:|:----:|------|
| 암호화 | AES-256-GCM 필수 | AES-256 확정 | XOR 스트림 | plan 수정: 현 방식 인정 (SHA-256 키 스트림) |
| 구독 가격 | 9,900/29,900 제안 | 미확정 | 4,900만 표시 | Phase 11에서 확정 |
| AI 분석 | 하이브리드 제안 | 동의 후 클라우드 | Mock만 구현 | Phase 10에 TODO 명시 |

### 12-4. 즉시 수정 가능한 빠른 수정 (Quick Fix)

| # | 작업 | 난이도 | 효과 |
|:-:|------|:------:|:----:|
| QF1 | 법률 챗봇 "99,000원 보고서" 문구 → "서류 작성" 링크로 변경 | 쉬움 | 높음 |
| QF2 | 홈 "증거 포렌식" 카드 설명 변경 ("증거 관리 & 분석") | 쉬움 | 중간 |
| QF3 | constants/police-report.ts 삭제 (사용처 없음 확인됨) | 쉬움 | 낮음 |
| QF4 | my-page "02-XXX-XXXX" → SOSModal 연결 (이미 구현됨, 확인만) | 쉬움 | 중간 |
| QF5 | plan.md 암호화 방식 현실 반영 ("SHA-256 키 스트림 기반 XOR 암호화") | 쉬움 | 낮음 |

---

## Phase 10: 증거 파일 업로드 + AI 자동 분석

> 기반: research.md 10장 리서치 결과

### 10-1. 개요

현재 증거보관함은 **텍스트 메모만** 저장 가능. 실제 파일(사진, 녹음, PDF)을 업로드하고 AI가 자동 분석하는 기능 추가.

```
파일 업로드 → 로컬 암호화 저장 → AI 분석(동의 시) → 법적 분류 + 위험도 + 법조항 매칭
```

### 10-2. 패키지 설치

```bash
npx expo install expo-image-picker expo-document-picker expo-audio expo-file-system
```
...
| 파일 | 역할 |
|------|------|
| `lib/file-manager.ts` | 파일 저장/읽기/삭제 (expo-file-system 래퍼) |
| `lib/ai-analysis.ts` | AI API 호출 + 분석 로직 (Claude Haiku + Whisper STT) |

### 10-4. 수정 파일

| 파일 | 수정 내용 |
|------|-----------|
| `lib/secure-evidence.ts` | `content` 필드에 `fileUri` 저장 지원 확장 |
| `app/(tabs)/evidence.tsx` | UPLOAD_BUTTONS에 실제 picker 연동 (현재 Alert만) |
| `app.json` | 카메라/마이크/파일 접근 권한 추가 |
| `constants/consent.ts` | AI 분석 동의 문구 추가 |

### 10-5. AI 분석 파이프라인

| 증거 유형 | 1차 (온디바이스) | 2차 (클라우드, 동의 시) | 결과 |
|-----------|:---------------:|:---------------------:|------|
| 카톡 캡처 | OCR 텍스트 추출 | Claude Haiku 법적 분석 | 협박/스토킹 분류 + 법조항 |
| 녹음 파일 | — | Whisper STT → Claude | 대화 내용 + 위협 감지 |
| 상해 사진 | — | Claude Vision | 상해 설명 + "진단서 권장" |
| PDF 문서 | — | Claude Sonnet | 문서 내용 분석 + 법적 의미 |
...
{
  aiAnalysis: {
    category: 'stalking',           // 법적 분류
    riskLevel: 'high',              // 위험도
    detectedKeywords: ['죽이겠다', '가만안둬'],
    applicableLaw: '형법 제283조 (협박)',
    summary: '피고소인이 살해 위협을 포함한 협박성 메시지를 전송',
    recommendation: '즉시 112 신고 + 증거 추가 확보 권장',
  }
}
```

### 10-7. 개인정보 동의 (하이브리드 방식)

- **1차 온디바이스**: 동의 불필요 (데이터 외부 전송 없음)
- **2차 클라우드**: **별도 동의 필수** "증거 파일이 AI 분석을 위해 외부 서비스로 전송됩니다"
- Claude API는 사용자 데이터를 모델 학습에 사용하지 않음
...
| 100명 x 10건 | ~$5-15 |
| 1,000명 x 10건 | ~$50-150 |
| 10,000명 x 10건 | ~$500-1,500 |

### 10-9. 구현 순서

| 단계 | 작업 | 의존성 |
|:----:|------|:------:|
| **P10-1** | 패키지 설치 + app.json 권한 추가 | 없음 |
| **P10-2** | `lib/file-manager.ts` 생성 (파일 저장/읽기) | P10-1 |
| **P10-3** | `evidence.tsx` 업로드 버튼 실제 picker 연동 | P10-1, P10-2 |
| **P10-4** | `secure-evidence.ts` fileUri 지원 확장 | P10-2 |
| **P10-5** | `lib/ai-analysis.ts` 생성 (Claude + Whisper API) | P10-4 |
| **P10-6** | 증거 카드에 AI 분석 결과 UI 표시 | P10-5 |
| **P10-7** | AI 분석 동의 모달 추가 | P10-5 |

### 10-10. 면책 고지

> "AI 분석 결과는 참고용이며, 법적 조언에 해당하지 않습니다. 원본 파일과 SHA-256 해시가 법적 증거로 활용됩니다."

---

## Phase 9: "나의 보호 현황" 실제 데이터 연동 + 멀티 트랙 프로세스

> 기반: research.md 9장 리서치 결과

### 9-1. 현재 문제

`app/(tabs)/my-page.tsx`의 모든 데이터가 하드코딩:
- 증거 `3건`, 체크리스트 `5/12`, 플랜 `FREE`, 진행 단계 고정, 연락처 `02-XXX-XXXX`
- 5단계 단일 트랙 (모든 사용자에게 동일)
...
| 항목 | 현재 | 변경 | 데이터 소스 |
|------|:----:|------|-----------|
| 저장된 증거 | `3건` | 실제 개수 | `loadAllEvidence().length` |
| 체크리스트 | `5/12` | 실제 진행률 | `AppContext.completedSteps` |
| 보호 플랜 | `FREE` | 실제 구독 상태 | AsyncStorage 구독 플래그 |
| 연락처 | `02-XXX-XXXX` | 실제 번호 또는 제거 | — |

**B) 멀티 트랙 프로세스 (단계적)**

현재 `StatusTracker` 컴포넌트의 5단계 → 사건 유형별 3트랙:

**트랙 A: 자가 보호 (무료)**
```
상황 파악 → 증거 수집(3건+) → 안전 계획 → 전문가 상담 → 지속 모니터링
```

**트랙 B: 경고장 발송 (유료)**
```
사실정리 → 변호사 검토 → 작성 완료 → 발송 → 수령 확인 → 답변 대기 → 후속 조치
```

**트랙 C: 고소/형사 (유료 프리미엄)**
```
증거 정리 → 고소장 작성 → 접수 → 수사 → 송치 → 기소 → 재판 → 판결
```

**C) 보호 점수 (0~100) — 향후**
```
보호 점수 = 증거 수집(25%) + 체크리스트(25%) + 안전계획(25%) + 법적 조치(25%)
+ "다음 행동 추천" 넛지: "증거 2건 더 수집하면 보호 점수 +5"
```
...
| 파일 | 수정 내용 |
|------|-----------|
| `app/(tabs)/my-page.tsx` | 하드코딩 제거, `loadAllEvidence()` 연동, 트랙 분기 UI |
| `components/StatusTracker.tsx` | `caseType` prop 추가, 트랙별 단계 표시 |
| `contexts/AppContext.tsx` | `caseType` 필드 추가 ('self_protect' / 'warning_letter' / 'complaint') |
| `types/database.ts` | `CaseType`, `ProtectionScore` 타입 추가 |

### 9-4. 구현 순서

| 단계 | 작업 | 의존성 |
|:----:|------|:------:|
| **P9-1** | `my-page.tsx` 하드코딩 제거 + `loadAllEvidence()` 연동 | 없음 |
| **P9-2** | `AppContext.tsx`에 `caseType` 추가 | 없음 |
| **P9-3** | `StatusTracker.tsx` 멀티 트랙 지원 | P9-2 |
| **P9-4** | 체크리스트 항목 정의 + `completedSteps` 연동 | P9-2 |
| **P9-5** | 보호 점수 산출 로직 + UI | P9-1, P9-4 |
| **P9-6** | 내용증명 발송 상태 추적 모델 | P9-3 |

---

## Phase 7: 통합 리팩토링 — 중복 제거 및 흐름 일원화 (✅ 완료)
